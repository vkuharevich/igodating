...
public class QuestionaryTemplateController {
    ...
    @GetMapping("/{id}")
    public ResponseWrapper<QuestionaryTemplateView> questionaryTemplate(@PathVariable("id") Long id) {
        return ResponseWrapper.ok(questionaryTemplateService.getById(id));
    }
    ...
    public ResponseWrapper<List<QuestionView>> questionsFromBlock(@PathVariable("questionBlockId") Long questionBlockId) {
        return ResponseWrapper.ok(questionaryTemplateService.getAllQuestionsFromBlock(questionBlockId));
    }
    ...
    public ResponseWrapper<List<QuestionView>> questionsWithoutBlock(@PathVariable("id") Long templateId) {
        return ResponseWrapper.ok(questionaryTemplateService.getAllQuestionsWithoutBlock(templateId));
    }

    @Operation(summary = "Блоки вопросов", description = "Получение всех блоков вопросов по ID шаблона")
    @GetMapping("/{id}/question-blocks")
    public ResponseWrapper<List<QuestionBlockView>> questionBlocks(@PathVariable("id") Long templateId) {
        return ResponseWrapper.ok(questionaryTemplateService.getAllQuestionBlocksByTemplateId(templateId));
    }

    @Operation(summary = "Создание шаблона", description = "Создает новый шаблон анкеты")
    @PostMapping
    public ResponseWrapper<Long> createTemplate(@RequestBody QuestionaryTemplateCreateRequest template) {
        return ResponseWrapper.ok(questionaryTemplateService.create(template));
    }

    @Operation(summary = "Обновление шаблона", description = "Обновление шаблона анкеты")
    @PutMapping
    public ResponseWrapper<Long> updateTemplate(@RequestBody QuestionaryTemplateUpdateRequest template) {
        return ResponseWrapper.ok(questionaryTemplateService.update(template));
    }

    @Operation(summary = "Удаление шаблона", description = "Удаление шаблона анкеты")
    @DeleteMapping
    public ResponseWrapper<Long> deleteTemplate(@RequestBody QuestionaryTemplateDeleteRequest template) {
        return ResponseWrapper.ok(questionaryTemplateService.delete(template));
    }
    ...
    public ResponseWrapper<Long> createQuestionBlock(@RequestBody QuestionBlockCreateDto questionBlock) {
        return ResponseWrapper.ok(questionaryTemplateService.createQuestionBlock(questionBlock));
    }
    ...
    public ResponseWrapper<Long> updateQuestionBlock(@RequestBody QuestionBlockUpdateDto questionBlock) {
        return ResponseWrapper.ok(questionaryTemplateService.updateQuestionBlock(questionBlock));
    }
}
